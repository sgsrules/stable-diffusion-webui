...
from deforum.src.infer import InferenceHelper
from deforum.src.midas.dpt_depth import DPTDepthModel
from deforum.src.midas.transforms import Resize, NormalizeImage, PrepareForNet
...
class DepthModel():
    def __init__(self, device):
        ...
        self.depth_min = 1000
        self.depth_max = -1000
        self.depth_minp = 1000
        self.depth_maxp = -1000
        self.device = device
    ...
    def predict(self, prev_img_cv2, midas_weight,zscale,zoffset) -> torch.Tensor:
        ...
        if self.midas_model is not None:
            ...
            mmin = midas_depth.min()
            mmax = midas_depth.max()
            # MiDaS makes the near values greater, and the far values lesser. Let's reverse that and try to align with AdaBins a bit better.
            #midas_depth = np.subtract(50.0, midas_depth)
            #midas_depth = midas_depth / 19.0
            self.depth_min = min(self.depth_min, mmin)
            self.depth_max = max(self.depth_max, mmax)

            midas_depth = (midas_depth - mmin) / (mmax- mmin)
            midas_depth = 1 - midas_depth
            midas_depth = midas_depth * zscale + zoffset
            self.depth_minp = min(self.depth_minp, midas_depth.min())
            self.depth_maxp = max(self.depth_maxp, midas_depth.max())

            print(f"  depth min:{self.depth_min} max:{self.depth_max} out min:{self.depth_minp} max: {self.depth_maxp}")
    ...
    def save(self, filename: str, depth: torch.Tensor):
        ...
        temp = rearrange(depth* 255, 'c h w -> h w c')
